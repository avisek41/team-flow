...
const SEED_FILE = process.argv[2] || '/Users/avisek/Desktop/dynamic_ui_final.json';
...
async function seed() {
  ...
  try {
    ...
    for (const [cityId, cityData] of Object.entries<any>(data.cities)) {
      ...
      const { error: cityErr } = await supabase.from('cities').upsert(cityPayload, { onConflict: 'id' });
      if (cityErr) console.error('Error seeding city:', cityErr);

      // 2. City Greetings
      for (const [phase, greeting] of Object.entries<any>(cityData.language.greetings)) {
      ...
      for (const [cycle, message] of Object.entries<any>(cityData.language.salary_messages)) {
      ...
      for (const [phase, categories] of Object.entries<any>(cityData.categories)) {
        for (let i = 0; i < categories.length; i++) {
          ...
          await supabase.from('city_categories').delete().eq('city_id', cityId).eq('phase_id', phase).eq('filter_value', cat.filter);
    ...
    for (const [festId, festData] of Object.entries<any>(data.festival_overlays)) {
      ...
      if (festData.greeting_override) {
        for (const [cityId, greeting] of Object.entries<any>(festData.greeting_override)) {
